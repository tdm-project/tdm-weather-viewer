...
var Windy = /** @class */ (function () {
	...
	this.geobounds = options.geobounds;
	...
	for (let j = 0; j<h; j++) {
	    for (let i = 0; i<w; i++) {
		let u = data[0][i + (h-1-j)*w];
		let v = -data[1][i + (h-1-j)*w];
		grid.push(new vector_1.default(u,v));
	    }
	}
	let la1=this.geobounds[1][0];
	let lo1=this.geobounds[0][1];
	let dy=(this.geobounds[1][0]-this.geobounds[0][0])/h;
	let dx=(this.geobounds[1][1]-this.geobounds[0][1])/w;
	let ny=h;
	let nx=w;

        this.grid = new grid_1.default(grid, la1, lo1, dy, dx, ny, nx);
        this.λ0 = lo1;
        this.φ0 = la1;
        this.Δλ = dx;
        this.Δφ = dy;
...
var AnimationBucket = /** @class */ (function () {
	...
	if (Math.abs(v.u)+Math.abs(v.v) > 0.1) {
            p.xt = p.x + v.u;
            p.yt = p.y + v.v;
